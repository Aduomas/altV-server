...
alt.onClient('chatmessage', (player, msg) => {
	if (msg[0] === '/') {
		...
		if (rangedChat) {
			var playersInRange = alt.Player.all.filter(x => Distance(player.pos, x.pos) <= rangeOfChat);

			if (playersInRange.length <= 0)
				return;

			var closePlayers = playersInRange.filter(x => Distance(player.pos, x.pos) <= (rangeOfChat / 2));
			var farPlayers = playersInRange.filter(x => Distance(player.pos, x.pos) >= (rangeOfChat / 2));

			closePlayers.forEach((target) => {
				alt.emitClient(target, 'chatmessage', player.name, msg.replace(/</g, '&lt;').replace(/'/g, '&#39').replace(/"/g, '&#34'));
			});

			farPlayers.forEach((target) => {
				alt.emitClient(target, 'chatmessage', null, `{707070} ${player.name}: ${msg.replace(/</g, '&lt;').replace(/'/g, '&#39').replace(/"/g, '&#34')}`);
			});
			return;
		}
